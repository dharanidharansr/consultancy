                    ...
                    <div className="space-y-4">
                                        ...
                                        availableColors.map((c, idx) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    aria-label={`Select variant ${c.color}`}
                                                    title={isDisabled ? 'Out of stock' : `${c.stock} in stock`}
                                                    disabled={isDisabled}
                                                >
                                                    {/* Hide color circle if it looks like a hex code but meant to be hidden, or just keep it as a dot if useful. For groceries, maybe just text is better? I'll keep the dot but maybe style it differently or remove if user wants text only. Let's keep existing logic but just rename label above. */}
                                                    <span
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                {/* Removed Size Guide Button */}
                            </div>
                            <p className="text-[11px] text-gray-500">
                                {(() => {
                                    const availableSizes = getAvailableSizes();
                                    return availableSizes.includes('1kg') ? 'Most popular: 1kg' : 'Select pack size';
                                })()}
                            </p>
                            <div className="flex flex-wrap gap-1.5">
                                {(() => {
                                    ...
                                    return availableSizes.length > 0 ? (
                                        availableSizes.map((size) => {
                                            ...
                                            const isLowStock = selectedColor && sizeStock > 0 && sizeStock <= 5;
                                            ...
                                            return (
                                                <div key={size} className="flex flex-col items-start">
                                                    <button
                                                        ...
                                                        disabled={isDisabled}
                                                        aria-pressed={isSelected}
                                                        title={isOutOfStock ? `${size} is out of stock` :
                                                            selectedColor ? `${sizeStock} ${size} available` :
