...
const OwnerInventory = () => {
    ...
    const handleStockChange = (variantIndex, packIndex, value) => {
        const newStock = [...stockData];
        newStock[variantIndex].packStock[packIndex].quantity = parseInt(value) || 0;
    ...
    const handleSaveStock = async () => {
        ...
        try {
            ...
            const updates = (stockData || []).flatMap((variantData, variantIndex) =>
                (variantData.packStock || []).map((packData, packIndex) => ({
                    variantIndex,
                    packIndex,
    ...
    return (
        <div className="p-6 w-full">
            <div className="mb-6">
                ...
                <p className="text-gray-600">Update product stock levels and variants</p>
            </div>
                    ...
                    {selectedProduct ? (
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            <div className="mb-6">
                                ...
                                <p className="text-gray-600">Update stock for variants and packs</p>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b bg-gray-50">
                                            ...
                                            {stockData?.[0]?.packStock?.map((item, idx) => (
                                                <th key={idx} className="px-4 py-2 text-center font-semibold">
                                                    {item.packSize || `Pack ${idx + 1}`}
                                                </th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {stockData?.map((variantData, variantIdx) => (
                                            <tr key={variantIdx} className="border-b hover:bg-gray-50">
                                                <td className="px-4 py-3">
                                                    <div className="flex items-center gap-2">
                                                        <div
                                                            className="w-6 h-6 rounded border"
                                                            style={{ backgroundColor: variantData.variant?.code || '#000' }}
                                                        />
                                                        <span className="font-medium">{variantData.variant?.name}</span>
                                                    </div>
                                                </td>
                                                {variantData.packStock?.map((packData, packIdx) => (
                                                    <td key={packIdx} className="px-4 py-3 text-center">
                                                        <input
                                                            type="number"
                                                            min="0"
                                                            value={packData.quantity || 0}
                                                            onChange={(e) => handleStockChange(variantIdx, packIdx, e.target.value)}
