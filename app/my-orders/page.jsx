...
const MyOrders = () => {
    ...
    return (
        <>
            <Navbar />
            <SEOMetadata
                title="My Orders | Glossary Mart"
                description="View and track your order history at Glossary Mart. Check order status, payment details, and delivery information."
                keywords="my orders, order history, purchase history, track orders"
                url="/my-orders"
            />
                    ...
                    {!loading && allOrders.length > 0 && (
                        <div className="flex flex-nowrap gap-2 mb-4 pb-3 border-b-2 border-gray-100 overflow-x-auto no-scrollbar">
                            {[
                                ...
                                { id: 'custom', label: 'Special Orders', icon: '📋', count: allOrders.filter(o => o.items && o.items.some(item => item.isCustomDesign)).length },
                                                                ...
                                                                {Array.isArray(order.items) && order.items.slice(0, 2).map((item, idx) => (
                                                                    <div key={idx} className="flex gap-3 bg-gray-50 rounded-lg p-2.5">
                                                                        ...
                                                                        <div className="flex-1 min-w-0">
                                                                            ...
                                                                            <p className="text-xs text-gray-600 mt-1">{item.size ? `Pack: ${item.size}` : ''} • Qty: {item.quantity || 1}</p>
                                                                        </div>
